...
for i in range(0,(len(collection_of_boundary))):
    ...
    if(i != (len(collection_of_boundary)-1)):
        #the argmax returns to the position in the specific numpy array, so the real position has to add the left boundary of the array
        ...
        #the reporting format of all peaks including noise
        collection_of_peak.append(('peak',i+1,'begins at time',t[collection_of_boundary[i][0][1]],'and ends at time',t[collection_of_boundary[i+1][0][1]],' peak',i+1,'time is',t[position_of_peak],' the corresponding absorbance value is',vector[position_of_peak]))
        
      
#sort the peaks by absorbance value and set the thereshold for y at 100, then screen out the 4 peaks        
collection_of_peak = sorted(collection_of_peak ,key=lambda x: x[11],reverse=True)      
threshold = [100]*180
for i in collection_of_peak:
    if(i[11]>threshold[0]):
        print(i[0],i[1],i[2],i[3],i[4],i[5],i[6],i[7],i[8],i[9],i[10],i[11])     
        plt.plot(i[9],i[11],'ro')   
                             
plt.plot(np.arange(0,180),threshold,'g--') 
plt.show()